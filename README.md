...
```bash
git clone https://github.com/mmanjula12/fda-competitive-intelligence-agent.git
cd fda-competitive-intelligence-agent
```
...
```bash
pip install anthropic mcp httpx flask python-dotenv
```
...
|---|---|
| `keytruda` | Merck's PD-1 inhibitor approvals, safety profile, competitive position |
| `nivolumab` | Bristol-Myers Squibb Opdivo approval history and trial insights |
| `Pfizer Inc.` | Full portfolio overview across all Pfizer FDA approvals |
| `Merck` | Merck sponsor-level competitive intelligence brief |

---

## How the Agent Works

1. User enters a drug name or sponsor in the browser
2. Flask backend receives the query and passes it to Claude
3. Claude decides which OpenFDA tools to call based on the query
4. The custom MCP server executes the FDA API calls and returns structured data
5. Claude reasons across all returned data and generates a four-section intelligence brief
6. The brief is rendered in the browser as a formatted report
...
Built by [@mmanjula12](https://github.com/mmanjula12) as part of a series of AI agent projects exploring the Anthropic Claude API and MCP server architecture.
